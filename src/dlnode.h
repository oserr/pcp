 ...
 */
template <typename T>
std::ostream &operator<<(std::ostream &os, const DlNode<T> &node) {
  os << "DlNode(" << node.value << ',' << node.prev << ',' << node.next << ')';
  return os;
}

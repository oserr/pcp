...
template<typename T>
bool
DlList<T>::Contains(T value) const noexcept
{
    auto node = head;
    while (node) {
        if (value == node->value)
...
template<typename T>
std::ostream&
operator<<(std::ostream &os, const DlList<T> &dlList)
{
    os << "DlList(";
...
template<typename T>
bool
operator==(const DlList<T> &lList, const DlList<T> &rList)
{
